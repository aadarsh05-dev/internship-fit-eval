...
def write_results(results, n, outdir=RESULTS_DIR):
    """Write the summary table and the v1/v3 disagreement detail to `outdir` as
    markdown, so a rubric change shows up as a reviewable diff in git."""
    os.makedirs(outdir, exist_ok=True)

    summary = [
        "# Eval results",
        "",
        f"Benchmark: {n} labeled postings. Regenerated by `python eval.py`.",
        "",
        "| version | exact-match | fit P | fit R | fit F1 | macro F1 |",
        "|---|---|---|---|---|---|",
    ]
    for name, (exact, cm, _diffs) in results.items():
        p, r, f1 = prf(cm, "fit")
        summary.append(
            f"| {name} | {exact}/{n} ({exact / n:.0%}) | {p:.0%} | {r:.0%} "
            f"| {f1:.2f} | {macro_f1(cm):.2f} |"
        )
    ...
    detail = [
        "# Disagreements",
        "",
        "Per-row disagreements for v1 (the starting rubric) and v3 (current).",
        "",
    ]
    for name in ("v1", "v3"):
        _exact, cm, diffs = results[name]
        detail += [
            f"## {name}",
            "",
            "Confusion matrix (rows = human label, columns = rubric):",
            "",
            "| human \\ rubric | " + " | ".join(CLASSES) + " |",
            "|---" * (len(CLASSES) + 1) + "|",
        ]
        for h in CLASSES:
            detail.append("| " + h + " | " + " | ".join(str(cm[(h, x)]) for x in CLASSES) + " |")
        detail += ["", f"{len(diffs)} disagreements:", ""]
        for cid, co, ro, human, pred in diffs:
            detail.append(f"- #{cid} {co} - {ro} - human=`{human}` rubric=`{pred}`")
        detail.append("")
    with open(os.path.join(outdir, "disagreements.md"), "w", encoding="utf-8") as f:
        f.write("\n".join(detail) + "\n")
